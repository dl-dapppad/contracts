...
describe('Payment, fork', async () => {
  ...
  before('setup', async () => {
    ...
    await upo.setFactory(FACTORY);
    await swap.swapWETH({ value: toWei('20').toString() });
  });

  describe('...', async () => {
    it('#1', async () => {
      const aaa = await upo.getSwapAmount(WETH, DAI, DAI, toWei('1').toString(), 500, 1000, 600);
      console.log(`${fromWei(aaa.toString()).toString()} ETH`);
      const bbb = await upo.getSwapAmount(WETH, DAI, WETH, toWei('10').toString(), 500, 1000, 600);
      console.log(`${fromWei(bbb.toString()).toString()} DAI`);
      const bbb1 = await upo.getSwapAmount(WETH, DAI, WETH, toWei('100').toString(), 500, 1000, 600);
      console.log(`${fromWei(bbb1.toString()).toString()} DAI`);
      const bbb2 = await upo.getSwapAmount(WETH, DAI, WETH, toWei('1000').toString(), 500, 1000, 600);
      console.log(`${fromWei(bbb2.toString()).toString()} DAI`);
    });
  });
