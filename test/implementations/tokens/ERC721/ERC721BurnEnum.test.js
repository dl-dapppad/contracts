...
const { ERC721BurnEnumDeployer } = require('../../../../helpers/deployers');

describe('ERC721BurnEnum', async () => {
  let erc721;
  let owner, ivan;
  ...
  beforeEach('setup', async () => {
    ...
    erc721 = await new ERC721BurnEnumDeployer().deployProxy(['N', 'S']);
  });

  describe('ERC721BurnEnum_init()', async () => {
    ...
    it('should revert on second initialize', async () => {
      await expect(erc721.proxy.ERC721BurnEnum_init('N', 'S')).to.be.revertedWith(
  ...
  describe('supportsInterface()', async () => {
    it('should set base URI', async () => {
      expect(await erc721.proxy.supportsInterface('0x80ac58cd')).to.be.equal(true); // IERC721Upgradeable
      expect(await erc721.proxy.supportsInterface('0x5b5e139f')).to.be.equal(true); // IERC721MetadataUpgradeable
      expect(await erc721.proxy.supportsInterface('0x780e9d63')).to.be.equal(true); // IERC721EnumerableUpgradeable
      expect(await erc721.proxy.supportsInterface('0x01ffc9a7')).to.be.equal(true); // IERC165Upgradeable
    });
  });
});
