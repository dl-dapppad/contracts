...
const { ERC20MintCappDeployer } = require('../../../../helpers/deployers');

describe('ERC20MintCapp', async () => {
  let erc20;
  let owner, ivan;
  ...
  beforeEach('setup', async () => {
    ...
    erc20 = await new ERC20MintCappDeployer().deployProxy(['N', 'S', '100', ivan.address, '8', '500']);
  });

  describe('ERC20MintCapp_init()', async () => {
    ...
    it('should initialize with zero amount', async () => {
      const erc20 = await new ERC20MintCappDeployer().deployProxy(['N', 'S', '0', ivan.address, '8', '500']);

      expect((await erc20.proxy.balanceOf(ivan.address)).toString()).to.be.equal('0');
    });
    it('should revert on second initialize', async () => {
      await expect(erc20.proxy.ERC20MintCapp_init('N', 'S', '0', ivan.address, '8', '500')).to.be.revertedWith(
