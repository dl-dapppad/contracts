...
const { ERC20MintDeployer } = require('../../../../helpers/deployers');

describe('ERC20Mint', async () => {
  let erc20;
  let owner, ivan;
  ...
  beforeEach('setup', async () => {
    ...
    erc20 = await new ERC20MintDeployer().deployProxy(['N', 'S', '100', ivan.address, '8']);
  });

  describe('ERC20Mint_init()', async () => {
    ...
    it('should initialize with zero amount', async () => {
      const erc20 = await new ERC20MintDeployer().deployProxy(['N', 'S', '0', ivan.address, '8']);

      expect((await erc20.proxy.balanceOf(ivan.address)).toString()).to.be.equal('0');
    });
    it('should revert on second initialize', async () => {
      await expect(erc20.proxy.ERC20Mint_init('N', 'S', '0', ivan.address, '8')).to.be.revertedWith(
